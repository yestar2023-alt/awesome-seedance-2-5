...
- [Katana-dance creator case — CR-74](./cases/creator-reported-x-2026-08-01-katana-dance.md): Kōda 公开的 30 秒高速舞刀首测与完整 prompt，连同已披露的形变/道具消失失败记录（`creator-reported`）。
- [Kestrel motorcycle creator case — CR-75](./cases/creator-reported-x-2026-07-31-kestrel-motorcycle.md): RafaSimon 公开的五参考图、12 镜头、720p/30 秒暗黑奇幻摩托追逐完整 prompt（`creator-reported`）。

目前共 **84 个**带可播放视频链接的案例；其中 **82 个**公开完整 prompt，**2 个**仅公开部分 prompt（CR-64、CR-68），覆盖：
